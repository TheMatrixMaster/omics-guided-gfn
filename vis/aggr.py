...
def load_run(run):
    ...
    for run_name, run_id in run_paths.items():
        ...
        top_k_modes_idx, top_k_modes_fps = find_modes_from_arrays(rewards, smis, fps, k=MAX_K, sim_threshold=0.7, return_fps=True)
        ...
        if should_plot_assay_preds:
            print(f"{run_name} topk rew assay preds: ", np.mean(top_k_reward_assay_preds), np.quantile(top_k_reward_assay_preds, 0.75), np.max(top_k_reward_assay_preds))
            print(f"{run_name} topk modes assay preds: ", np.mean(top_k_modes_assay_preds), np.quantile(top_k_modes_assay_preds, 0.75), np.max(top_k_modes_assay_preds))
            print(f"{run_name} topk tsim assay preds: ", np.mean(top_k_tsim_assay_preds), np.quantile(top_k_tsim_assay_preds, 0.75), np.max(top_k_tsim_assay_preds))
...
def merge(runs_datum, joint_datum):
...
def go(joint_datum, num_runs, save_dir=None):
    print(f"Producing plots for aggregated targets under mode {TARGET_MODE}...")
    ...
    plot_tsim_and_reward_full_hist(joint_datum, bins=50,
                                   ...
                                   save_path=f"{save_dir}/full_tsim_reward_hist.png")
    
    # PLOTS THAT REQUIRE DUPLICATES REMOVED
    plot_assay_cluster_preds_hist(joint_datum, k=MAX_K*num_runs, bins=50,
        ignore=["PUMA_test"], save_path=f"{save_dir}/assay_cluster_preds_hist.png")
    plot_tsim_between_modes_and_to_target(joint_datum, k1=MAX_K*num_runs, k2=2000, bins=50, 
                                    save_path=f"{save_dir}/tsim_modes_hist.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--normalize_preds", action="store_true", help="Normalize assay and cluster predictions")
    ...
    parser.add_argument("--config_name", type=str, default="morph_runs_aggr.json", help="JSON config to use")
    ...
    parser.add_argument("--run_name", type=str, default="run1", help="Run name to use")
    ...
    for run in RUNS:
        ...
        if args.plot_individual:
            ...
            save_dir = f"{SAVEDIR}/{target_idx}-{TARGET_MODE}-{RUN_NAME}"
            if USE_GNEPROP: save_dir += "-gneprop"
            if args.normalize_preds: save_dir += "-norm"
            go(runs_datum, 1, save_dir)
    ...
    save_dir = f"{SAVEDIR}/aggr-{TARGET_MODE}-{RUN_NAME}"
    if USE_GNEPROP: save_dir += "-gneprop"
    if args.normalize_preds: save_dir += "-norm"
    go(joint_datum, NUM_RUNS, save_dir)